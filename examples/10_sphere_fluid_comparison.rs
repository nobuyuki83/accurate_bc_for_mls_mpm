    ...
    -> Vec::<Real>
{
    ...
    for i in 0..n_div {
        let x = Real::cos(std::f64::consts::PI * 2.0 * (i as Real) / (n_div as Real));
        let y = Real::sin(std::f64::consts::PI * 2.0 * (i as Real) / (n_div as Real));
        coords.push(radius * x + x_center);
        coords.push(radius * y + y_center);
    }
...
fn sim(is_ours: bool, is_full_slip: bool, path: &str) {
    ...
    let mut bg = {
        let boundary = 0.047;
        let vtx2xy_boundary = generate_sphere_boundary(0.45, 0.5, 0.5, 64);
        // [
        // //boundary, boundary,
        // boundary, 0.5,
        // 1. - boundary, boundary,
...
fn main() {
    ...
    sim(false, false,"target/10_naive_nonslip_sphere.gif");
    println!("naive {:?}", now.elapsed());
    let now = time::Instant::now();
    sim(true, false,"target/10_ours_nonslip_sphere.gif");
    println!("our {:?}", now.elapsed());
    //
    sim(true, true,"target/10_ours_fullslip_sphere.gif");
}