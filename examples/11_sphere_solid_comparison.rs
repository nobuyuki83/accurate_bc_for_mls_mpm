...
fn sim(is_ours: bool, is_snow: bool, is_full_slip: bool, path: &str) {
    ...
    const N: usize = 80;
    let mut bg = {
        let vtx2xy_boundary = generate_sphere_boundary(0.45, 0.5, 0.5, 128);
...
fn main() {
    sim(true, true, true, "target/11_our_snow_sphere.gif");
    sim(true, false, true,"target/11_our_hyper_sphere.gif");
}